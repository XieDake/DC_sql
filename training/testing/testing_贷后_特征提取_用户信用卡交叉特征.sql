...
#提取用户贷后交叉特征！综合！
#暂时13维！
drop table if exists features_3_later;
create table features_3_later
(
...
# insert
insert into features_3_later
(
select
t1.user_id,
#13维交叉特征！
(max(t1.bill_time_stamp)-min(t2.loan_time))/(3600*24),#天为单位！
...
features_1_later as t3,
features_2_later as t4
#
where
...
insert into features_3_later (features_3_later.user_id)
(#返回overdue中有但是features中没有的user！
select distinct(t1.user_id) from userid_predict_final as t1
where
(t1.user_id not in (select distinct(user_id) from features_3_later))
);